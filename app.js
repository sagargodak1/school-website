...
/* =========================================================
   FOUNDER + PARENT PORTALS — FINAL READ-ONLY ADD-ON
   Added after every existing website module so old workflows stay unchanged.
========================================================= */
(function(){
  "use strict";
  const PORTAL_CLASSES=["Nursery","LKG","UKG","Class 1","Class 2","Class 3","Class 4","Class 5","Class 6","Class 7","Class 8","Class 9","Class 10"];
  const portalState={
    founder:{token:sessionStorage.getItem("saaf_founder_portal_token")||"",data:null,tab:"overview"},
    parent:{token:sessionStorage.getItem("saaf_parent_portal_token")||"",data:null,tab:"home",photosLoaded:false},
    admin:{rows:[],cycles:[],publication:null,founders:[]}
  };

  function esc(v){return typeof window.escapeHtml==="function"?window.escapeHtml(String(v??"")):String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));}
  function portalDb(){const db=typeof window.initStudentSupabase==="function"?window.initStudentSupabase():null;if(!db)throw new Error("Supabase connection is unavailable.");return db;}
  async function rpc(name,args={}){const {data,error}=await portalDb().rpc(name,args);if(error)throw error;return data;}
  function dateText(v){if(!v)return "—";try{return new Date(v).toLocaleString("en-GB",{timeZone:"Asia/Kathmandu",day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:true});}catch(_){return String(v)}}
  function num(v,d=2){const n=Number(v);if(!Number.isFinite(n))return "—";return Number.isInteger(n)?String(n):n.toFixed(d).replace(/0+$/,'').replace(/\.$/,'');}
  function portalMessage(role,text,type="error"){
    const el=document.getElementById(`${role}PortalLoginMessage`);if(!el)return;el.textContent=text||"";el.className=`portal-message ${text?"show":""} ${type}`;
  }
  function setBusy(id,busy,busyText){const b=document.getElementById(id);if(!b)return;if(!b.dataset.oldText)b.dataset.oldText=b.textContent;b.disabled=!!busy;b.textContent=busy?(busyText||"PLEASE WAIT…"):b.dataset.oldText;}
  function showPopup(id){const el=document.getElementById(id);if(el)el.style.display="block";document.body.style.overflow="hidden";}
  function showFlex(id){const el=document.getElementById(id);if(el)el.style.display="flex";document.body.style.overflow="hidden";}
  window.portalClose=function(id){const el=document.getElementById(id);if(el)el.style.display="none";document.body.style.overflow="auto";};

  window.portalOpenLogin=async function(role){
    if(!["founder","parent"].includes(role))return;
    const st=portalState[role];
    if(st.token){
      try{if(role==="founder"){await portalLoadFounder(false);showPopup("founderPortalPopup");}else{await portalLoadParent(false);showPopup("parentPortalPopup");}return;}catch(_){sessionStorage.removeItem(`saaf_${role}_portal_token`);st.token="";st.data=null;}
    }
    portalMessage(role,"");
    showFlex(role==="founder"?"founderLoginPopup":"parentLoginPopup");
    setTimeout(()=>document.getElementById(role==="founder"?"founderPortalId":"parentPortalId")?.focus(),100);
  };

  window.portalLogin=async function(role){
    if(!["founder","parent"].includes(role))return;
    const idEl=document.getElementById(role==="founder"?"founderPortalId":"parentPortalId"),pwEl=document.getElementById(role==="founder"?"founderPortalPassword":"parentPortalPassword");
    const loginId=String(idEl?.value||"").trim(),password=String(pwEl?.value||"");
    if(!loginId||!password){portalMessage(role,"Enter Login ID and password.");return;}
    const btnId=role==="founder"?"founderPortalLoginBtn":"parentPortalLoginBtn";setBusy(btnId,true,"SIGNING IN…");portalMessage(role,"Checking secure access…","success");
    let signedIn=false;
    try{
      const data=await rpc("portal_login",{p_login_id:loginId,p_password:password,p_expected_role:role});
      if(!data?.token)throw new Error("Login could not be completed.");
      portalState[role].token=data.token;sessionStorage.setItem(`saaf_${role}_portal_token`,data.token);if(pwEl)pwEl.value="";
      signedIn=true;
      portalClose(role==="founder"?"founderLoginPopup":"parentLoginPopup");
      if(role==="founder"){
        showPopup("founderPortalPopup");
        await portalLoadFounder(false);
      }else{
        portalState.parent.photosLoaded=false;
        showPopup("parentPortalPopup");
        await portalLoadParent(false);
      }
    }catch(error){
      if(!signedIn){
        portalState[role].token="";sessionStorage.removeItem(`saaf_${role}_portal_token`);portalMessage(role,error?.message||"Login failed. Check ID and password.");
      }else if(!portalState[role].token){
        const dashId=role==="founder"?"founderPortalPopup":"parentPortalPopup";
        document.getElementById(dashId)?.style.setProperty("display","none");
        showFlex(role==="founder"?"founderLoginPopup":"parentLoginPopup");
        portalMessage(role,error?.message||"Portal session expired. Please login again.");
      }else{
        portalSmallToast(role==="founder"?"Founder dashboard opened. Some data could not be loaded.":"Parent dashboard opened. Some data could not be loaded.");
      }
    }finally{setBusy(btnId,false);}
  };

  window.portalLogout=async function(role){
    if(!["founder","parent"].includes(role))return;const st=portalState[role],token=st.token;st.token="";st.data=null;if(role==="parent")st.photosLoaded=false;sessionStorage.removeItem(`saaf_${role}_portal_token`);
    document.getElementById(role==="founder"?"founderPortalPopup":"parentPortalPopup")?.style.setProperty("display","none");document.body.style.overflow="auto";
    if(token){try{await rpc("portal_logout",{p_token:token});}catch(_){}}
  };

  // ---------------- FOUNDER ----------------
  window.portalLoadFounder=async function(showToast=false){
    const token=portalState.founder.token;if(!token)throw new Error("Founder login is required.");
    const host=document.getElementById("founderPortalContent");if(host)host.innerHTML='<div class="portal-loading">Loading Founder Dashboard…</div>';
    try{portalState.founder.data=await rpc("portal_founder_dashboard",{p_token:token});portalRenderFounderSummary();portalRenderFounderTab();if(showToast)portalSmallToast("Founder dashboard refreshed.");return portalState.founder.data;}
    catch(error){if(/session|login|expired|invalid/i.test(error?.message||"")){sessionStorage.removeItem("saaf_founder_portal_token");portalState.founder.token="";}if(host)host.innerHTML=`<div class="portal-empty">${esc(error?.message||"Founder Dashboard could not be loaded.")}</div>`;throw error;}
  };
  function portalRenderFounderSummary(){
    const d=portalState.founder.data||{},host=document.getElementById("founderPortalSummary");if(!host)return;const pub=d.result_publication;
    host.innerHTML=`<div class="portal-stat"><small>Total Students</small><strong>${esc(d.total_students??0)}</strong><span>Nursery to Class 10</span></div><div class="portal-stat"><small>Announcements</small><strong>${esc((d.announcements||[]).length)}</strong><span>Latest Principal updates</span></div><div class="portal-stat"><small>Upcoming Plans</small><strong>${esc((d.plans||[]).length)}</strong><span>School plans in view</span></div><div class="portal-stat"><small>Parent Result</small><strong>${pub?.is_published?"LIVE":"OFF"}</strong><span>${pub?.is_published?`${esc(pub.academic_session)} • ${esc(pub.term_name)}`:"Not published"}</span></div>`;
  }
  window.portalFounderTab=function(tab,button){portalState.founder.tab=tab;document.querySelectorAll('#founderPortalPopup [data-founder-tab]').forEach(b=>b.classList.toggle('active',b===button));portalRenderFounderTab();};
  function portalRenderFounderTab(){
    const d=portalState.founder.data||{},host=document.getElementById("founderPortalContent");if(!host)return;const tab=portalState.founder.tab;
    if(tab==="announcements"){host.innerHTML=portalFounderAnnouncements(d.announcements||[]);return;}
    if(tab==="plans"){host.innerHTML=portalFounderPlans(d.plans||[]);return;}
    if(tab==="marks"){host.innerHTML=portalFounderMarks(d.marks_summary||[]);return;}
    if(tab==="exam"){host.innerHTML=portalFounderExam(d.exam_status||[]);return;}
    host.innerHTML=`<div class="portal-grid-2"><div class="portal-card"><h3>📢 Latest Announcements</h3>${portalFounderAnnouncements(d.announcements||[],3,true)}</div><div class="portal-card"><h3>📅 Upcoming Plans</h3>${portalFounderPlans(d.plans||[],3,true)}</div><div class="portal-card"><h3>📊 Academic Monitoring</h3><p>Founder sees Class-wise Marks summary only. Individual student marks are never sent to this portal.</p>${portalFounderMarks((d.marks_summary||[]).slice(0,6),true)}</div><div class="portal-card"><h3>📝 Exam Monitoring</h3>${portalFounderExam(d.exam_status||[],true)}</div></div>`;
  }
  function portalFounderAnnouncements(rows,limit=50,embedded=false){const data=rows.slice(0,limit);const html=data.length?`<div class="portal-list">${data.map(r=>`<div class="portal-list-item"><strong>${esc(r.title||"Announcement")}</strong><span class="portal-priority ${esc(String(r.priority||"").toLowerCase())}">${esc(r.priority||"Normal")}</span><small>${esc(r.message||"")}${r.event_date_bs?`<br>Event: ${esc(r.event_date_bs)}${r.event_day?` • ${esc(r.event_day)}`:""}`:""}</small></div>`).join("")}</div>`:'<div class="portal-empty">No announcement found.</div>';return embedded?html:`<div class="portal-card"><h3>Principal Announcements</h3>${html}</div>`;}
  function portalFounderPlans(rows,limit=50,embedded=false){const data=rows.slice(0,limit);const html=data.length?`<div class="portal-list">${data.map(r=>`<div class="portal-list-item"><strong>${esc(r.plan_name||"Plan")}</strong><small>${esc(r.plan_date_bs||"Date not set")} • Incharge: ${esc(r.incharge_name||"—")}${r.remarks?`<br>${esc(r.remarks)}`:""}</small></div>`).join("")}</div>`:'<div class="portal-empty">No upcoming plan found.</div>';return embedded?html:`<div class="portal-card"><h3>Upcoming Plans</h3>${html}</div>`;}
  function portalFounderMarks(rows,embedded=false){
    const html=rows.length?`<div class="portal-table-wrap"><table class="portal-table"><thead><tr><th>Class</th><th>Academic Year</th><th>Term</th><th>Subjects</th><th>Class Average</th><th>Attendance</th><th>Admin Received</th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${esc(r.class_name)}</strong></td><td>${esc(r.academic_session)}</td><td>${esc(r.term_name)}</td><td>${esc(r.subject_count??0)}</td><td>${r.class_percentage==null?"—":`${esc(num(r.class_percentage))}%`}</td><td>${r.attendance_percentage==null?"—":`${esc(num(r.attendance_percentage))}%`}</td><td>${esc(dateText(r.submitted_to_admin_at))}</td></tr>`).join("")}</tbody></table></div>`:'<div class="portal-empty">No Class package has reached Admin yet.</div>';
    return embedded?html:`<div class="portal-card"><h3>Class-wise Marks Summary</h3><p>No individual student result is available in Founder access.</p>${html}</div>`;
  }
  function portalFounderExam(rows,embedded=false){const html=rows.length?`<div class="portal-list">${rows.map(r=>`<div class="portal-list-item"><strong>${esc(r.academic_year)} • ${esc(r.term_name)}</strong><span class="portal-status good">Published</span><small>${esc(r.exam_title||"Exam Routine")} • Questions submitted ${esc(r.submitted_questions??0)}/${esc(r.required_questions??0)} • Printed ${esc(r.printed_questions??0)}/${esc(r.required_questions??0)}</small></div>`).join("")}</div>`:'<div class="portal-empty">No published Exam Routine found.</div>';return embedded?html:`<div class="portal-card"><h3>Exam Routine & Question Status</h3>${html}</div>`;}

  // ---------------- PARENT ----------------
  window.portalLoadParent=async function(showToast=false){
    const token=portalState.parent.token;if(!token)throw new Error("Parent login is required.");const host=document.getElementById("parentPortalContent");if(host)host.innerHTML='<div class="portal-loading">Loading your child’s information…</div>';
    try{portalState.parent.data=await rpc("portal_parent_dashboard",{p_token:token});await portalRenderParentHero();portalRenderParentTab();if(showToast)portalSmallToast("Parent portal refreshed.");return portalState.parent.data;}
    catch(error){if(/session|login|expired|invalid/i.test(error?.message||"")){sessionStorage.removeItem("saaf_parent_portal_token");portalState.parent.token="";}if(host)host.innerHTML=`<div class="portal-empty">${esc(error?.message||"Parent Portal could not be loaded.")}</div>`;throw error;}
  };
  async function portalRenderParentHero(){
    const d=portalState.parent.data||{},p=d.profile||{},host=document.getElementById("parentPortalHero");if(!host)return;
    host.innerHTML=`<div class="portal-parent-photo" id="parentPortalPhoto">👤</div><div class="portal-parent-copy"><div class="portal-kicker" style="color:#bff5f3">PRIVATE STUDENT PORTAL</div><h2>${esc(p.name||"Student")}</h2><div class="portal-parent-meta"><span class="portal-parent-chip">${esc(p.class_name||"Class")}</span><span class="portal-parent-chip">Student ID: ${esc(p.student_id||"—")}</span><span class="portal-parent-chip">Read Only</span></div></div>`;
    try{if(p.student_id&&typeof window.cloudSignedUrl==="function"){const url=await window.cloudSignedUrl(`students/${p.student_id}/profile.jpg`,900);if(url){const box=document.getElementById("parentPortalPhoto");if(box)box.innerHTML=`<img src="${esc(url)}" alt="${esc(p.name||"Student")}">`;}}}catch(_){ }
  }
  window.portalParentTab=function(tab,button){portalState.parent.tab=tab;document.querySelectorAll('#parentPortalPopup [data-parent-tab]').forEach(b=>b.classList.toggle('active',b===button));portalRenderParentTab();};
  function portalRenderParentTab(){
    const d=portalState.parent.data||{},host=document.getElementById("parentPortalContent");if(!host)return;const tab=portalState.parent.tab;
    if(tab==="result"){host.innerHTML=portalParentResult(d);return;}
    if(tab==="attendance"){host.innerHTML=portalParentAttendance(d.attendance,d.publication);return;}
    if(tab==="routine"){host.innerHTML=portalParentRoutine(d.routine);return;}
    if(tab==="notices"){host.innerHTML=portalParentNotices(d.notices||[]);return;}
    if(tab==="progress"){host.innerHTML=portalParentProgressShell(d);portalLoadParentProgressImages();return;}
    host.innerHTML=portalParentHome(d);
  }
  function portalParentHome(d){const p=d.profile||{},pub=d.publication,att=d.attendance;return `<div class="portal-grid-2"><div class="portal-card"><h3>Student Profile</h3><div class="portal-list"><div class="portal-list-item"><strong>${esc(p.name||"—")}</strong><small>${esc(p.class_name||"—")} • ${esc(p.student_id||"—")}</small></div><div class="portal-list-item"><strong>Date of Birth (B.S.)</strong><small>${esc(p.dob||"To be updated")}</small></div><div class="portal-list-item"><strong>Parent / Guardian</strong><small>${esc(p.guardian||"To be updated")}</small></div>${p.performance||p.remarks?`<div class="portal-list-item"><strong>Teacher Remark / Performance</strong><small>${esc(p.performance||"")}${p.performance&&p.remarks?" • ":""}${esc(p.remarks||"")}</small></div>`:""}</div></div><div class="portal-card"><h3>Quick Access</h3><div class="portal-quick-grid"><button class="portal-quick" onclick="portalOpenParentNamedTab('result')">📊<b>My Result</b><span>${pub?.is_published?`${esc(pub.term_name)} published`:"Waiting for Admin publish"}</span></button><button class="portal-quick" onclick="portalOpenParentNamedTab('attendance')">📅<b>Attendance</b><span>${att?.total_opening_days?`${esc(att.present_days)} / ${esc(att.total_opening_days)} days`:"View attendance"}</span></button><button class="portal-quick" onclick="portalOpenParentNamedTab('progress')">📷<b>Monthly Progress</b><span>Month 1–12 portfolio photos</span></button><button class="portal-quick" onclick="portalOpenParentNamedTab('routine')">📝<b>Exam Routine</b><span>Own class routine only</span></button><button class="portal-quick" onclick="portalOpenParentNamedTab('notices')">📢<b>Notices</b><span>Latest school notices</span></button></div></div></div>`;}
  window.portalOpenParentNamedTab=function(tab){const b=document.querySelector(`#parentPortalPopup [data-parent-tab="${tab}"]`);if(b)b.click();};
  function portalParentResult(d){const pub=d.publication,res=d.result;if(!pub?.is_published)return '<div class="portal-card"><h3>📊 My Result</h3><div class="portal-empty">Result has not been published by the Admin yet.</div></div>';if(!res?.subjects?.length)return `<div class="portal-card"><h3>📊 My Result</h3><div class="portal-empty">${esc(pub.academic_session)} • ${esc(pub.term_name)} is published, but no final result was found for this student.</div></div>`;const subjectHtml=res.subjects.map(s=>`<div class="portal-result-subject"><div class="portal-result-subject-head"><strong>${esc(s.subject_name)}</strong><span>${esc(num(s.obtained_marks))} / ${esc(num(s.full_marks))}</span></div>${(s.components||[]).map(c=>`<div class="portal-component-row"><span>${esc(c.component_name)}</span><span>FM ${esc(num(c.full_marks))}</span><span class="${c.is_absent?'portal-abs':''}">${c.is_absent?'ABS':`OM ${esc(num(c.obtained_marks))}`}</span></div>`).join("")}</div>`).join("");return `<div class="portal-card"><div class="portal-result-hero"><div class="portal-result-title"><div class="portal-kicker">PUBLISHED RESULT</div><h3>${esc(pub.term_name)} • ${esc(pub.academic_session)}</h3><p>Only this logged-in student’s final Admin-published result is shown.</p></div><div class="portal-score-ring"><strong>${res.percentage==null?"—":`${esc(num(res.percentage))}%`}</strong><small>${esc(num(res.overall_obtained))} / ${esc(num(res.overall_full))}</small></div></div>${subjectHtml}<div style="display:flex;justify-content:flex-end;margin-top:14px"><button class="portal-action-btn teal" onclick="portalPrintParentResult()">🖨 PRINT / SAVE PDF</button></div><p style="font-size:11px;margin-top:12px">ABS is displayed separately. Totals use the recorded obtained marks.</p></div>`;}
  function portalParentAttendance(a,pub){if(!pub?.is_published)return '<div class="portal-card"><h3>📅 Attendance</h3><div class="portal-empty">Attendance for the result cycle becomes visible after Admin publishes the result.</div></div>';if(!a?.total_opening_days)return '<div class="portal-card"><h3>📅 Attendance</h3><div class="portal-empty">Attendance is not available for this published result.</div></div>';return `<div class="portal-card"><h3>📅 Attendance — ${esc(pub.term_name)}</h3><div class="portal-attendance-big"><div><small>Opening Days</small><strong>${esc(a.total_opening_days)}</strong></div><div><small>Present Days</small><strong>${esc(a.present_days)}</strong></div><div><small>Absent Days</small><strong>${esc(a.absent_days)}</strong></div><div><small>Attendance</small><strong>${esc(num(a.percentage))}%</strong></div></div></div>`;}
  function portalParentRoutine(r){if(!r?.rows?.length)return '<div class="portal-card"><h3>📝 Exam Routine</h3><div class="portal-empty">No published routine is available for this class.</div></div>';return `<div class="portal-card"><h3>📝 ${esc(r.exam_title||"Exam Routine")}</h3><p>${esc(r.academic_year)} • ${esc(r.term_name)}</p><div class="portal-table-wrap"><table class="portal-table"><thead><tr><th>Day</th><th>Date</th><th>Subject / Activity</th><th>Teacher</th></tr></thead><tbody>${r.rows.map(x=>`<tr><td><strong>${esc(x.day_label)}</strong></td><td>${esc(x.exam_date||"—")}</td><td>${esc(x.activity_title||"—")}</td><td>${esc(x.teacher_name||"—")}</td></tr>`).join("")}</tbody></table></div></div>`;}
  function portalParentNotices(rows){const html=rows.length?`<div class="portal-list">${rows.map(n=>`<div class="portal-list-item"><strong>📢 School Notice</strong><small>${esc(n.notice_text||"")}</small></div>`).join("")}</div>`:'<div class="portal-empty">No notice is available.</div>';return `<div class="portal-card"><h3>School Notices</h3>${html}</div>`;}
  function portalParentProgressShell(d){const year=d.current_academic_year||d.publication?.academic_session||"";return `<div class="portal-card"><h3>📷 Monthly Progress</h3><p>${year?`Academic Year ${esc(year)} • `:""}Only this student’s Month 1–12 portfolio photos.</p><div id="parentPortalProgressGrid" class="portal-progress-grid">${Array.from({length:12},(_,i)=>`<div class="portal-progress-tile" id="parentPortalMonth${i+1}"><div class="portal-progress-missing">Loading Month ${i+1}…</div><span class="portal-progress-label">Month ${i+1}</span></div>`).join("")}</div></div>`;}
  async function portalLoadParentProgressImages(){const d=portalState.parent.data||{},p=d.profile||{},year=d.current_academic_year||d.publication?.academic_session||"";if(!p.student_id||typeof window.cloudSignedUrl!=="function")return;await Promise.all(Array.from({length:12},async(_,i)=>{const month=i+1,box=document.getElementById(`parentPortalMonth${month}`);if(!box)return;try{let path=year?`students/${p.student_id}/${year}/month-${month}.jpg`:`students/${p.student_id}/month-${month}.jpg`,url=await window.cloudSignedUrl(path,900);if(!url&&String(year)==="2083")url=await window.cloudSignedUrl(`students/${p.student_id}/month-${month}.jpg`,900);if(url)box.innerHTML=`<img src="${esc(url)}" alt="Month ${month}" onclick="openPhoto(this.src)"><span class="portal-progress-label">Month ${month}</span>`;else box.innerHTML=`<div class="portal-progress-missing">Month ${month}<br>Not uploaded</div><span class="portal-progress-label">Month ${month}</span>`;}catch(_){box.innerHTML=`<div class="portal-progress-missing">Month ${month}<br>Not available</div><span class="portal-progress-label">Month ${month}</span>`;}}));portalState.parent.photosLoaded=true;}
  window.portalPrintParentResult=function(){const d=portalState.parent.data||{},p=d.profile||{},pub=d.publication,res=d.result;if(!pub?.is_published||!res?.subjects?.length)return;const rows=res.subjects.map(s=>`<tr><td><b>${esc(s.subject_name)}</b></td><td>${esc((s.components||[]).map(c=>`${c.component_name} (${num(c.full_marks)})`).join(" + "))}</td><td>${esc((s.components||[]).map(c=>c.is_absent?"ABS":num(c.obtained_marks)).join(" + "))}</td><td>${esc(num(s.full_marks))}</td><td>${esc(num(s.obtained_marks))}</td></tr>`).join("");const w=window.open("","_blank","width=900,height=700");if(!w)return;w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(p.name||"Student")} Result</title><style>body{font-family:Arial,sans-serif;padding:28px;color:#17324b}h1,h2,p{text-align:center;margin:6px}table{width:100%;border-collapse:collapse;margin-top:22px;font-size:12px}th,td{border:1px solid #667;padding:8px;text-align:center}th{background:#eee}.meta{margin-top:14px;font-weight:bold}.sum{text-align:right;margin-top:14px;font-weight:bold}@page{size:A4;margin:14mm}</style></head><body><h1>ST. AUGUSTINE ACADEMIC FOUNDATION</h1><h2>${esc(pub.term_name)} — Result</h2><p>${esc(pub.academic_session)}</p><div class="meta">Student: ${esc(p.name)} &nbsp; | &nbsp; ID: ${esc(p.student_id)} &nbsp; | &nbsp; Class: ${esc(p.class_name)}</div><table><thead><tr><th>Subject</th><th>Components (FM)</th><th>Obtained / ABS</th><th>Subject FM</th><th>Subject OM</th></tr></thead><tbody>${rows}</tbody></table><div class="sum">Overall: ${esc(num(res.overall_obtained))} / ${esc(num(res.overall_full))} ${res.percentage==null?"":`(${esc(num(res.percentage))}%)`}</div><script>window.onload=function(){window.print()}<\/script></body></html>`);w.document.close();};

  // ---------------- ADMIN PORTAL CONTROL ----------------
  function portalEnsureAdminTab(){
    const panel=document.getElementById("websiteAdminPanel"),tabs=panel?.querySelector(".website-admin-tabs");if(!panel||!tabs)return;
    if(!document.getElementById("portalAdminTabButton")){const b=document.createElement("button");b.id="portalAdminTabButton";b.type="button";b.innerHTML="👥 Founder & Parent Portal";b.onclick=()=>window.showWebsiteAdminTab("portalAccess");tabs.appendChild(b);}
    if(document.getElementById("websiteAdminPortalAccessTab"))return;
    const div=document.createElement("div");div.id="websiteAdminPortalAccessTab";div.className="website-admin-tab";div.style.display="none";div.innerHTML=`<div class="admin-section-head"><div><h3>👥 Founder & Parent Portal</h3><p>Manage Founder and Parent portal accounts and publish one Result cycle to all Nursery–Class 10 parents.</p></div><button type="button" class="admin-mini-btn" onclick="portalAdminLoad()">↻ Refresh</button></div><div id="portalAdminMessage" class="portal-message"></div><div class="portal-admin-shell"><section class="portal-admin-card"><div class="portal-founder-head"><div><h4>🏛️ Founder Login Accounts</h4><p>Create up to 10 Founder Login IDs. Every Founder opens the same school-wide Founder Portal; Marks remain Class-wise only.</p></div><span id="portalAdminFounderCount" class="portal-founder-count">0 / 10</span></div><div class="portal-founder-create"><label class="portal-field">New Founder Login ID<input id="portalAdminNewFounderId" placeholder="e.g. FOUNDER01"></label><label class="portal-field">Password<input id="portalAdminNewFounderPassword" type="password" autocomplete="new-password" placeholder="Minimum 8 characters"></label><button id="portalAdminCreateFounderBtn" class="portal-action-btn gold" type="button" onclick="portalAdminCreateFounder()">+ CREATE FOUNDER</button></div><div id="portalAdminFounderList" class="portal-admin-founder-list"><div class="portal-empty">Loading Founder accounts…</div></div></section><section class="portal-admin-card"><h4>📊 Result Publication — One Switch for All Classes</h4><p>Select Academic Year and Term. Publish is blocked until all 13 Classes have submitted their final Marks + Attendance package to Admin.</p><div class="portal-admin-pub"><label class="portal-field">Academic Year<input id="portalAdminResultYear" list="portalAdminYearList" placeholder="2083"></label><label class="portal-field">Term / Examination<input id="portalAdminResultTerm" list="portalAdminTermList" placeholder="First Terminal Examination"></label><button class="portal-action-btn teal" type="button" onclick="portalAdminPublishResults()">PUBLISH RESULT — NURSERY TO CLASS 10</button><button class="portal-danger-btn" type="button" onclick="portalAdminUnpublishResults()">UNPUBLISH</button></div><datalist id="portalAdminYearList"></datalist><datalist id="portalAdminTermList"></datalist><div id="portalAdminPublicationStatus" class="portal-admin-current">Loading publication status…</div><div id="portalAdminCycles" class="portal-admin-cycle-list"></div></section><section class="portal-admin-card"><h4>👨‍👩‍👧 Parent Accounts</h4><p>Each Parent Login is linked to exactly one Student ID. Parent can never browse another student.</p><div class="portal-admin-roster-tools"><select id="portalAdminClassFilter" onchange="portalAdminRenderParents()"><option value="ALL">All Classes</option>${PORTAL_CLASSES.map(c=>`<option>${esc(c)}</option>`).join("")}</select><input id="portalAdminParentSearch" type="search" placeholder="Search student / ID / parent login" oninput="portalAdminRenderParents()"><button class="portal-ghost-btn" type="button" onclick="portalAdminLoad()">↻ Refresh</button></div><div id="portalAdminParentList" class="portal-admin-roster"><div class="portal-empty">Loading students…</div></div></section></div>`;panel.appendChild(div);
  }
  function portalAdminMsg(text,type="error"){const el=document.getElementById("portalAdminMessage");if(!el)return;el.textContent=text||"";el.className=`portal-message ${text?"show":""} ${type}`;}
  window.portalAdminLoad=async function(){if(!studentAdminSession?.access_token){openStudentAdminLogin();return;}portalEnsureAdminTab();const list=document.getElementById("portalAdminParentList"),founderList=document.getElementById("portalAdminFounderList");if(list)list.innerHTML='<div class="portal-loading">Loading portal accounts…</div>';if(founderList)founderList.innerHTML='<div class="portal-loading">Loading Founder accounts…</div>';try{const d=await rpc("portal_admin_overview");portalState.admin.rows=d?.parents||[];portalState.admin.cycles=d?.cycles||[];portalState.admin.publication=d?.publication||null;portalState.admin.founders=Array.isArray(d?.founders)?d.founders:(d?.founder?[d.founder]:[]);portalAdminRenderFounders();portalAdminRenderPublication();portalAdminRenderParents();portalAdminMsg("Portal controls are ready.","success");}catch(error){portalAdminMsg((error?.message||"Portal database is not ready.")+" Run FOUNDER_MULTI_ACCOUNT_PATCH.sql once if you already installed the older Portal SQL.");if(list)list.innerHTML='<div class="portal-empty">Portal setup is not ready.</div>';if(founderList)founderList.innerHTML='<div class="portal-empty">Founder account controls are not ready.</div>';}};
  function portalAdminRenderFounders(){const rows=portalState.admin.founders||[],host=document.getElementById("portalAdminFounderList"),count=document.getElementById("portalAdminFounderCount"),createBtn=document.getElementById("portalAdminCreateFounderBtn");if(count)count.textContent=`${rows.length} / 10`;if(createBtn){createBtn.disabled=rows.length>=10;createBtn.textContent=rows.length>=10?"LIMIT REACHED":"+ CREATE FOUNDER";}if(!host)return;host.innerHTML=rows.length?rows.map((f,i)=>`<div class="portal-admin-founder"><div class="portal-founder-info"><strong>Founder ${i+1}</strong><span class="portal-status ${f.active?'good':'warn'}">${f.active?'ACTIVE':'DISABLED'}</span><div class="meta">Current ID: <b>${esc(f.login_id)}</b><br>Updated ${esc(dateText(f.updated_at))}</div></div><input id="portalFounderEditId_${Number(f.account_id)}" value="${esc(f.login_id)}" placeholder="Founder Login ID"><input id="portalFounderEditPass_${Number(f.account_id)}" type="password" autocomplete="new-password" placeholder="New password (optional)"><div class="portal-founder-actions"><button class="portal-action-btn teal" type="button" onclick="portalAdminUpdateFounder(${Number(f.account_id)})">SAVE / RESET</button><button class="${f.active?'portal-danger-btn':'portal-ghost-btn'}" type="button" onclick="portalAdminToggleFounder(${Number(f.account_id)})">${f.active?'DISABLE':'ENABLE'}</button><button class="portal-delete-btn" type="button" onclick="portalAdminDeleteFounder(${Number(f.account_id)})">DELETE</button></div></div>`).join(""):'<div class="portal-empty">No Founder Login has been created yet. Create the first one above.</div>';}
  function portalAdminRenderPublication(){const p=portalState.admin.publication,status=document.getElementById("portalAdminPublicationStatus"),cycles=portalState.admin.cycles||[];if(status)status.innerHTML=p?.is_published?`<span class="portal-status good">LIVE TO ALL PARENTS</span> <strong>${esc(p.academic_session)} • ${esc(p.term_name)}</strong> • Published ${esc(dateText(p.published_at))}`:'<span class="portal-status warn">NOT PUBLISHED</span> Parents cannot see any Result yet.';const yearList=document.getElementById("portalAdminYearList"),termList=document.getElementById("portalAdminTermList"),host=document.getElementById("portalAdminCycles");if(yearList)yearList.innerHTML=[...new Set(cycles.map(c=>c.academic_session))].map(v=>`<option value="${esc(v)}"></option>`).join("");if(termList)termList.innerHTML=[...new Set(cycles.map(c=>c.term_name))].map(v=>`<option value="${esc(v)}"></option>`).join("");if(host)host.innerHTML=cycles.slice(0,12).map(c=>`<span class="portal-admin-cycle">${esc(c.academic_session)} • ${esc(c.term_name)} • ${esc(c.ready_classes)}/13 ready</span>`).join("");}
  window.portalAdminRenderParents=function(){const host=document.getElementById("portalAdminParentList");if(!host)return;const cls=document.getElementById("portalAdminClassFilter")?.value||"ALL",q=String(document.getElementById("portalAdminParentSearch")?.value||"").trim().toLowerCase();const rows=(portalState.admin.rows||[]).filter(r=>(cls==="ALL"||r.class_name===cls)&&(!q||String(r.student_name).toLowerCase().includes(q)||String(r.student_id).toLowerCase().includes(q)||String(r.login_id||"").toLowerCase().includes(q)));host.innerHTML=rows.length?rows.map(r=>{const sid=esc(r.student_id),defaultId=`P-${String(r.student_id).replace(/[^a-zA-Z0-9-]/g,"")}`;return `<div class="portal-admin-student"><div><strong>${esc(r.student_name)}</strong><div class="meta">${esc(r.class_name)} • ${sid}</div></div><div><span class="portal-status ${r.account_active?'good':'warn'}">${r.login_id?(r.account_active?'ACTIVE':'DISABLED'):'NO LOGIN'}</span><div class="meta">${r.login_id?`Current: ${esc(r.login_id)}`:'Create account below'}</div></div><input id="portalParentLogin_${sid}" value="${esc(r.login_id||defaultId)}" placeholder="Parent Login ID"><input id="portalParentPass_${sid}" type="password" autocomplete="new-password" placeholder="${r.login_id?'New password (optional)':'Password (8+)'}"><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="portal-action-btn teal" type="button" onclick="portalAdminSaveParent('${sid}')">${r.login_id?'SAVE / RESET':'CREATE LOGIN'}</button>${r.login_id?`<button class="${r.account_active?'portal-danger-btn':'portal-ghost-btn'}" type="button" onclick="portalAdminToggleParent('${sid}',${r.account_active?'false':'true'})">${r.account_active?'DISABLE':'ENABLE'}</button>`:""}</div></div>`;}).join(""):'<div class="portal-empty">No students match this filter.</div>';};
  window.portalAdminCreateFounder=async function(){if(!studentAdminSession?.access_token)return;const loginId=document.getElementById("portalAdminNewFounderId")?.value.trim()||"",password=document.getElementById("portalAdminNewFounderPassword")?.value||"";if((portalState.admin.founders||[]).length>=10){alert("Maximum 10 Founder Login accounts are allowed.");return;}if(loginId.length<3){alert("Enter a Founder Login ID with at least 3 characters.");return;}if(password.length<8){alert("Founder password must have at least 8 characters.");return;}if(!confirm(`Create Founder Login ID: ${loginId}?`))return;try{await rpc("portal_admin_create_founder",{p_login_id:loginId,p_password:password});document.getElementById("portalAdminNewFounderId").value="";document.getElementById("portalAdminNewFounderPassword").value="";await portalAdminLoad();alert("Founder Login created successfully.");}catch(error){alert("Founder Login could not be created: "+(error?.message||"Unknown error"));}};
  window.portalAdminUpdateFounder=async function(accountId){const f=(portalState.admin.founders||[]).find(x=>Number(x.account_id)===Number(accountId));if(!f)return;const loginId=document.getElementById(`portalFounderEditId_${accountId}`)?.value.trim()||"",password=document.getElementById(`portalFounderEditPass_${accountId}`)?.value||"";if(loginId.length<3){alert("Founder Login ID must have at least 3 characters.");return;}if(password&&password.length<8){alert("New password must have at least 8 characters.");return;}if(!confirm(`Save changes for Founder Login ${f.login_id}?${password?"\n\nThis password reset will sign out the current session.":""}`))return;try{await rpc("portal_admin_update_founder",{p_account_id:Number(accountId),p_login_id:loginId,p_password:password||null});await portalAdminLoad();alert(password?"Founder ID / Password updated successfully.":"Founder Login ID saved successfully.");}catch(error){alert("Founder account could not be updated: "+(error?.message||"Unknown error"));}};
  window.portalAdminToggleFounder=async function(accountId){const f=(portalState.admin.founders||[]).find(x=>Number(x.account_id)===Number(accountId));if(!f)return;const active=!f.active;if(!confirm(`${active?"Enable":"Disable"} Founder Login ${f.login_id}?${active?"":"\n\nThat Founder will be signed out immediately."}`))return;try{await rpc("portal_admin_toggle_account",{p_account_id:Number(accountId),p_active:active});await portalAdminLoad();}catch(error){alert("Founder Login status could not be changed: "+(error?.message||"Unknown error"));}};
  window.portalAdminDeleteFounder=async function(accountId){const f=(portalState.admin.founders||[]).find(x=>Number(x.account_id)===Number(accountId));if(!f)return;if(!confirm(`DELETE FOUNDER LOGIN\n\nID: ${f.login_id}\n\nThis removes only this Founder Login account and signs it out. It does NOT delete any school data. Continue?`))return;try{await rpc("portal_admin_delete_founder",{p_account_id:Number(accountId)});await portalAdminLoad();alert("Founder Login deleted successfully.");}catch(error){alert("Founder Login could not be deleted: "+(error?.message||"Unknown error"));}};
  window.portalAdminSaveParent=async function(studentId){if(!studentAdminSession?.access_token)return;const row=(portalState.admin.rows||[]).find(r=>String(r.student_id)===String(studentId)),loginId=document.getElementById(`portalParentLogin_${studentId}`)?.value.trim()||"",password=document.getElementById(`portalParentPass_${studentId}`)?.value||"";if(!loginId){alert("Enter Parent Login ID.");return;}if(!row?.login_id&&password.length<8){alert("For a new Parent account, enter a password with at least 8 characters.");return;}if(password&&password.length<8){alert("Password must have at least 8 characters.");return;}try{await rpc("portal_admin_save_account",{p_role:"parent",p_student_id:studentId,p_login_id:loginId,p_password:password||null,p_active:true});await portalAdminLoad();alert("Parent Login saved successfully.");}catch(error){alert("Parent Login could not be saved: "+(error?.message||"Unknown error"));}};
  window.portalAdminToggleParent=async function(studentId,active){const row=(portalState.admin.rows||[]).find(r=>String(r.student_id)===String(studentId));if(!row?.account_id)return;if(!confirm(`${active?"Enable":"Disable"} Parent Login for ${row.student_name}?`))return;try{await rpc("portal_admin_toggle_account",{p_account_id:Number(row.account_id),p_active:!!active});await portalAdminLoad();}catch(error){alert("Account status could not be changed: "+(error?.message||"Unknown error"));}};
  window.portalAdminPublishResults=async function(){const year=document.getElementById("portalAdminResultYear")?.value.trim()||"",term=document.getElementById("portalAdminResultTerm")?.value.trim()||"";if(!year||!term){alert("Enter Academic Year and Term / Examination.");return;}if(!confirm(`PUBLISH RESULT TO ALL PARENTS\n\nAcademic Year: ${year}\nTerm: ${term}\n\nThis is one global publication for Nursery to Class 10. Continue?`))return;try{const result=await rpc("portal_admin_publish_results",{p_academic_session:year,p_term_name:term});await portalAdminLoad();alert(`✓ Result published for all ${result?.classes_published||13} Classes. Each Parent can see only their own child.`);}catch(error){alert("Result was NOT published:\n\n"+(error?.message||"Unknown error"));}};
  window.portalAdminUnpublishResults=async function(){if(!confirm("Unpublish the current Parent Result now? Parents will immediately stop seeing Result and cycle Attendance."))return;try{await rpc("portal_admin_unpublish_results");await portalAdminLoad();alert("Parent Result unpublished.");}catch(error){alert("Could not unpublish: "+(error?.message||"Unknown error"));}};

  function portalSmallToast(text){const old=document.getElementById("portalTinyToast");old?.remove();const el=document.createElement("div");el.id="portalTinyToast";el.textContent=text;Object.assign(el.style,{position:"fixed",right:"18px",bottom:"18px",zIndex:"150000",background:"#123c69",color:"white",padding:"11px 14px",borderRadius:"10px",boxShadow:"0 8px 24px rgba(0,0,0,.24)",fontWeight:"700",fontSize:"13px"});document.body.appendChild(el);setTimeout(()=>el.remove(),2200);}

  // Add the new Admin tab after the final existing showWebsiteAdminTab wrapper.
  let adminWrapped=false;
  function wrapAdminTabs(){portalEnsureAdminTab();if(adminWrapped||typeof window.showWebsiteAdminTab!=="function")return;const previous=window.showWebsiteAdminTab;window.showWebsiteAdminTab=function(tab){portalEnsureAdminTab();previous(tab);const el=document.getElementById("websiteAdminPortalAccessTab");if(el)el.style.display=tab==="portalAccess"?"block":"none";if(tab==="portalAccess")portalAdminLoad();};adminWrapped=true;}

  document.addEventListener("DOMContentLoaded",()=>{
    wrapAdminTabs();
    document.getElementById("founderPortalPassword")?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();portalLogin("founder");}});
    document.getElementById("parentPortalPassword")?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();portalLogin("parent");}});
  });
})();
