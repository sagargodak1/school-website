...
(function(){
  ...
  const portalState={
    ...
    admin:{rows:[],cycles:[],publication:null,founder:null}
  };
  ...
  function portalEnsureAdminTab(){
    ...
    const div=document.createElement("div");div.id="websiteAdminPortalAccessTab";div.className="website-admin-tab";div.style.display="none";div.innerHTML=`<div class="admin-section-head"><div><h3>👥 Founder & Parent Portal</h3><p>Manage read-only portal accounts and publish one Result cycle to all Nursery–Class 10 parents.</p></div><button type="button" class="admin-mini-btn" onclick="portalAdminLoad()">↻ Refresh</button></div><div id="portalAdminMessage" class="portal-message"></div><div class="portal-admin-shell"><section class="portal-admin-card"><h4>🏛️ Shared Founder Login</h4><p>Manage the shared Founder Login ID, password and access status. Founder Marks remain Class-wise only.</p><div class="portal-admin-row"><label class="portal-field">Founder Login ID<input id="portalAdminFounderId" placeholder="FOUNDERS"></label><label class="portal-field">New Password<input id="portalAdminFounderPassword" type="password" autocomplete="new-password" placeholder="Leave blank to keep current"></label><button class="portal-action-btn gold" type="button" onclick="portalAdminSaveFounder()">SAVE / RESET</button><button id="portalAdminFounderToggle" class="portal-danger-btn" type="button" onclick="portalAdminToggleFounder()" style="display:none">DISABLE</button></div><div id="portalAdminFounderStatus" class="portal-admin-current">No Founder account loaded.</div></section><section class="portal-admin-card"><h4>📊 Result Publication — One Switch for All Classes</h4><p>Select Academic Year and Term. Publish is blocked until all 13 Classes have submitted their final Marks + Attendance package to Admin.</p><div class="portal-admin-pub"><label class="portal-field">Academic Year<input id="portalAdminResultYear" list="portalAdminYearList" placeholder="2083"></label><label class="portal-field">Term / Examination<input id="portalAdminResultTerm" list="portalAdminTermList" placeholder="First Terminal Examination"></label><button class="portal-action-btn teal" type="button" onclick="portalAdminPublishResults()">PUBLISH RESULT — NURSERY TO CLASS 10</button><button class="portal-danger-btn" type="button" onclick="portalAdminUnpublishResults()">UNPUBLISH</button></div><datalist id="portalAdminYearList"></datalist><datalist id="portalAdminTermList"></datalist><div id="portalAdminPublicationStatus" class="portal-admin-current">Loading publication status…</div><div id="portalAdminCycles" class="portal-admin-cycle-list"></div></section><section class="portal-admin-card"><h4>👨‍👩‍👧 Parent Accounts</h4><p>Each Parent Login is linked to exactly one Student ID. Parent can never browse another student.</p><div class="portal-admin-roster-tools"><select id="portalAdminClassFilter" onchange="portalAdminRenderParents()"><option value="ALL">All Classes</option>${PORTAL_CLASSES.map(c=>`<option>${esc(c)}</option>`).join("")}</select><input id="portalAdminParentSearch" type="search" placeholder="Search student / ID / parent login" oninput="portalAdminRenderParents()"><button class="portal-ghost-btn" type="button" onclick="portalAdminLoad()">↻ Refresh</button></div><div id="portalAdminParentList" class="portal-admin-roster"><div class="portal-empty">Loading students…</div></div></section></div>`;panel.appendChild(div);
  }
  ...
  window.portalAdminLoad=async function(){if(!studentAdminSession?.access_token){openStudentAdminLogin();return;}portalEnsureAdminTab();const list=document.getElementById("portalAdminParentList");if(list)list.innerHTML='<div class="portal-loading">Loading portal accounts…</div>';try{const d=await rpc("portal_admin_overview");portalState.admin.rows=d?.parents||[];portalState.admin.cycles=d?.cycles||[];portalState.admin.publication=d?.publication||null;portalState.admin.founder=d?.founder||null;portalAdminRenderFounder();portalAdminRenderPublication();portalAdminRenderParents();portalAdminMsg("Portal controls are ready.","success");}catch(error){portalAdminMsg((error?.message||"Portal database is not ready.")+" Run founder_parent_portal_setup.sql once in Supabase.");if(list)list.innerHTML='<div class="portal-empty">Portal setup is not ready.</div>';}};
  function portalAdminRenderFounder(){const f=portalState.admin.founder,id=document.getElementById("portalAdminFounderId"),status=document.getElementById("portalAdminFounderStatus"),toggle=document.getElementById("portalAdminFounderToggle");if(id&&!id.value)id.value=f?.login_id||"FOUNDERS";if(status)status.innerHTML=f?`Status: <strong>${f.active?"ACTIVE":"DISABLED"}</strong> • Login ID: <strong>${esc(f.login_id)}</strong> • Updated ${esc(dateText(f.updated_at))}`:'No Founder login has been created yet.';if(toggle){toggle.style.display=f?"inline-flex":"none";toggle.textContent=f?.active?"DISABLE":"ENABLE";toggle.className=f?.active?"portal-danger-btn":"portal-ghost-btn";}}
  ...
  window.portalAdminSaveFounder=async function(){if(!studentAdminSession?.access_token)return;const loginId=document.getElementById("portalAdminFounderId")?.value.trim()||"",password=document.getElementById("portalAdminFounderPassword")?.value||"";if(!loginId){alert("Enter Founder Login ID.");return;}if(!portalState.admin.founder&&password.length<8){alert("For a new Founder account, enter a password with at least 8 characters.");return;}if(password&&password.length<8){alert("Password must have at least 8 characters.");return;}if(!confirm("Save the shared Founder Login?"))return;try{const keepActive=portalState.admin.founder?!!portalState.admin.founder.active:true;await rpc("portal_admin_save_account",{p_role:"founder",p_student_id:null,p_login_id:loginId,p_password:password||null,p_active:keepActive});document.getElementById("portalAdminFounderPassword").value="";await portalAdminLoad();alert(password?"Founder Login / Password updated successfully.":"Founder Login saved successfully.");}catch(error){alert("Founder Login could not be saved: "+(error?.message||"Unknown error"));}};
  window.portalAdminToggleFounder=async function(){const f=portalState.admin.founder;if(!f?.account_id)return;const active=!f.active;if(!confirm(`${active?"Enable":"Disable"} the shared Founder Login?${active?"":"\n\nAny current Founder session will be signed out immediately."}`))return;try{await rpc("portal_admin_toggle_account",{p_account_id:Number(f.account_id),p_active:active});await portalAdminLoad();alert(`Founder Login ${active?"enabled":"disabled"} successfully.`);}catch(error){alert("Founder Login status could not be changed: "+(error?.message||"Unknown error"));}};
