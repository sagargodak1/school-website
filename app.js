...
(function(){
  ...
  function portalAdminRenderFounder(){const f=portalState.admin.founder,id=document.getElementById("portalAdminFounderId"),status=document.getElementById("portalAdminFounderStatus");if(id&&!id.value)id.value=f?.login_id||"FOUNDERS";if(status)status.innerHTML=f?`Status: <strong>${f.active?"ACTIVE":"DISABLED"}</strong> • Login ID: <strong>${esc(f.login_id)}</strong> • Updated ${esc(dateText(f.updated_at))}`:'No Founder login has been created yet.';}
